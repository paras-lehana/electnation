...
| **Testing** | Vitest unit + Supertest integration now implemented; service-level Forward Clinic/security tests added; component/e2e queued | `pnpm --filter @yatra/functions test`, `pnpm --filter @yatra/core test`, direct web/functions `tsc --noEmit`; full build queued after security slice | `TESTING.md` |
